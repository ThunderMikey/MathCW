clear;
init_condi=@(x)2*x.*(x<0.5)+(2-2*x).*(x>=0.5); %declare initial condition
boundry_left=@(x)10*x;						%declare left boundary condition
boundry_right=@(x)10*x;					%declare right boundary condition
...
str=sprintf('1-D heat equation with tent initial conditions and time-varying \n boundary conditions, %d iterations in time', iterations);
title(str);
xlabel('distance');
ylabel('temperature');
